...
const DesignPreview = ({ configuration }: { configuration: Configuration }) => {
  ...
  const [shippingAddressFilled, setShippingAddressFilled] = useState<boolean>(false);

  // State for shippingAddress
  ...
  const handleShippingFormSubmit = (address: any) => {
  ...
  const handleCheckout = () => {
    if (user) {
      // Ensure shipping address is filled out
      if (!shippingAddressFilled) {
        toast({
          title: 'Incomplete shipping address',
          description: 'Please fill out all required fields in the shipping address form.',
          variant: 'destructive'
        });
        return;
      }
      ...
      createPaymentSession({ configId: id, shippingAddress: shippingAddress });
    } else {
      // need to log in
  ...
  return (
    <>
      <div aria-hidden='true' className='pointer-events-none select-none absolute inset-0 overflow-hidden flex justify-center'>
        <Confetti active={showConfetti} config={{ elementCount: 500, spread: 400 }} />
      </div>

      <LoginModal isOpen={isLoginModalOpen} setIsOpen={setIsLoginModalOpen} />
      <ShippingAddress
        isOpen={isShippingFormOpen}
        setIsOpen={setIsShippingFormOpen}
        handleSubmit={handleShippingFormSubmit}
      />
          ...
          <div className='mt-8'>
            ...
            <div className='mt-8 flex flex-col items-end pb-12'>
              <div className='flex flex-row'>
                <Button onClick={() => setIsShippingFormOpen(true)} className='mr-4 px-4 sm:px-6 lg:px-8'>
                  Add Shipping Address
                </Button>
                <Button onClick={handleCheckout} className='px-4 sm:px-6 lg:px-8' disabled={!shippingAddressFilled}>
